...
var futureWeatherContainerEl = document.querySelector(
...
var displayTodayWeather = function (weather, weatherData) {
  ...
  displayEl.innerHTML = `Temperature: ${Math.floor(
    (weather.main.temp - 273.15) * 1.8 + 32
  )} °F <br> Humidity: ${weather.main.humidity} %<br> Wind Speed: ${
    weather.wind.speed
  } MPH<br>`;
...
var displayFutureForecast = function (weatherForecast, weatherDataForecast) {
  ...
  for (
    var i = 0;
    i < weatherForecast.list.length && numDaysDisplayed < 5;
    i++
  ) {
    var dateTime = weatherForecast.list[i].dt_txt;
    var temp = Math.floor(
      (weatherForecast.list[i].main.temp - 273.15) * 1.8 + 32
    );
    ...
    if (previousDate != parsedDate && parsedDate != currentDate) {
      ...
      <img id=\"wicon\" src=\"https://openweathermap.org/img/wn/${icon}.png\" width=\"50px\" height=\"50px\" alt=\"icon\"></img>\
      <p>Temp: ${temp} °F </p>\
